...
public sealed class TrySwitchingLoadoutPatch : BasePatch {
    ...
    private void IL_Player_TrySwitchingLoadout(ILContext il) {
        ...
        c.EmitDelegate<Func<int, Player, int>>((loadoutIndex, player) => {
            ...
            if (player.GetModPlayer<LoadoutPlayer>().CurrentExtraLoadoutIndex >= 0) {
        ...
        //if (!c.TryGotoNext(MoveType.After,
        //        opcode => opcode.MatchCall<Player>(nameof(Player.CloneLoadouts))
        //)) {
        //    throw new Exception("Failed while patching TrySwitchingLayout: could not match (call Player.CloneLoadouts)");
        //}

        //c.Emit(OpCodes.Ldarg_0);
        //c.EmitDelegate<Action<Player>>((player) => {
        //    if (Main.netMode != NetmodeID.SinglePlayer) {
        //        player.GetModPlayer<LoadoutPlayer>().CopyClientState(Main.clientPlayer.GetModPlayer<LoadoutPlayer>());
        //    }
        //});
    }
}
